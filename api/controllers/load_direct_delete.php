...
$empQuery = "SELECT *, direct.id as direct_id, direct.qnt as direct_qnt , direct.type as type FROM direct LEFT JOIN cars ON direct.sn=cars.sn AND cars.vessel_id = $vessel_id  WHERE direct.vessel_id= $vessel_id  AND is_delete = 0 AND   $searchQuery order by    $columnName $columnSortOrder  $rowLength ";
